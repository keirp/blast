...
class RandomVideoSource(ImageSource):
    def __init__(self, shape, filelist, mode='play'):
        """
        Args:
            shape: [h, w]
        ...
        """
        self.shape_wh = shape[::-1]
        self.frame_list = [skvideo.io.vread(fname) for fname in filelist]
        self.reset()
        self.mode = mode

    def reset(self):
        idx = random.randint(0, len(self.frame_list) - 1)
        self.frames = self.frame_list[idx]
        self.n_frames = self.frames.shape[0]
        self.frame_idx = random.randint(0, self.n_frames - 1)
